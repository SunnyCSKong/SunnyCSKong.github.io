/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
Command: npx gltfjsx@6.4.1 public/models/solar_system_animation.glb 
Author: Samer_Arab_S5 (https://sketchfab.com/Samer_Arab_S5)
License: CC-BY-4.0 (http://creativecommons.org/licenses/by/4.0/)
Source: https://sketchfab.com/3d-models/solar-system-animation-b7c69a6b655b47c99f871d5ec5aee854
Title: Solar System animation
*/

import React, { useState, useCallback } from "react";
import { useGLTF, useAnimations } from "@react-three/drei";
import { debounce } from "lodash";
import { Names } from "./Names";
import { useThree } from "@react-three/fiber";
import { Select } from "@react-three/postprocessing";
export function System(props) {
  const group = React.useRef();
  const { nodes, materials, animations } = useGLTF(
    "./models/solar_system_animation.glb"
  );
  const { camera } = useThree();

  const [hovered, hover] = useState(null);
  const [highlightHovered, highlightHover] = useState(null);
  const debouncedHover = useCallback(debounce(hover, 30), []);
  const over = (name) => (e) => (e.stopPropagation(), debouncedHover(name));

  const { actions } = useAnimations(animations, group);

  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Sketchfab_Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]}>
          <group name="root">
            <group name="GLTF_SceneRootNode" rotation={[Math.PI / 2, 0, 0]}>
              <group name="mercury_BezierCircle_4" scale={9.695}>
                <Select
                  enabled={hovered === "Mercury"}
                  onPointerOver={over("Mercury")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="mercury_2"
                    position={[0, 0, 1]}
                    rotation={[1.739, -0.875, 1.237]}
                    scale={0.038}
                  >
                    <mesh
                      name="Object_5"
                      geometry={nodes.Object_5.geometry}
                      material={materials.mercury}
                      onPointerOver={() => highlightHover(true)}
                      onPointerOut={() => highlightHover(false)}
                    />
                  </group>
                </Select>
              </group>
              <group
                name="venus_BezierCircle_7"
                rotation={[0, Math.PI, 0]}
                scale={12.235}
              >
                <Select
                  enabled={hovered === "Venus"}
                  onPointerOver={over("Venus")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="venus_5"
                    position={[0, 0, 1]}
                    rotation={[0, 0, Math.PI / 2]}
                    scale={0.102}
                  >
                    <mesh
                      name="Object_8"
                      geometry={nodes.Object_8.geometry}
                      material={materials.venus}
                    />
                  </group>
                </Select>
              </group>
              <group name="erath_BezierCircle_11" scale={16.115}>
                <Select
                  enabled={hovered === "Earth"}
                  onPointerOver={over("Earth")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="erath_8"
                    position={[0.84, 0, 0.55]}
                    rotation={[0, 1, 0]}
                    scale={0.062}
                  >
                    <mesh
                      name="Object_11"
                      geometry={nodes.Object_11.geometry}
                      material={materials.earth}
                    />
                  </group>
                </Select>
              </group>
              <group name="mars_BezierCircle_14" scale={20.451}>
                <Select
                  enabled={hovered === "Mars"}
                  onPointerOver={over("Mars")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="mars_12"
                    position={[0.45, 0, 0.89]}
                    rotation={[0, 0, Math.PI / 2]}
                    scale={0.025}
                  >
                    <mesh
                      name="Object_14"
                      geometry={nodes.Object_14.geometry}
                      material={materials.mars}
                    />
                  </group>
                </Select>
              </group>
              <group name="jupiter_BezierCircle_17" scale={28.775}>
                <Select
                  enabled={hovered === "Jupiter"}
                  onPointerOver={over("Jupiter")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="jupiter_15"
                    position={[0.55, 0, -0.838]}
                    scale={0.095}
                  >
                    <mesh
                      name="Object_17"
                      geometry={nodes.Object_17.geometry}
                      material={materials.jupiter}
                    />
                  </group>
                </Select>
              </group>
              <group name="saturn_BezierCircle_21" scale={36.61}>
                <Select
                  enabled={hovered === "Saturn"}
                  onPointerOver={over("Saturn")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="saturn_19"
                    position={[0.87, 0, -0.5]}
                    scale={0.059}
                  >
                    <group
                      name="saturn_ring_18"
                      position={[0.01, -0.067, 0]}
                      rotation={[0, 0, 0.351]}
                      scale={1.739}
                    >
                      <mesh
                        name="Object_22"
                        geometry={nodes.Object_22.geometry}
                        material={materials.saturn_ring}
                      />
                    </group>
                    <mesh
                      name="Object_20"
                      geometry={nodes.Object_20.geometry}
                      material={materials.saturn}
                    />
                  </group>
                </Select>
              </group>
              <group name="uranus_BezierCircle_24" scale={44.26}>
                <Select
                  enabled={hovered === "Uranus"}
                  onPointerOver={over("Uranus")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="uranus_22"
                    position={[0.35, 0, 0.935]}
                    rotation={[0, 0, Math.PI / 2]} //
                    scale={0.02}
                  >
                    <mesh
                      name="Object_25"
                      geometry={nodes.Object_25.geometry}
                      material={materials.uranus}
                    />
                  </group>
                </Select>
              </group>
              <group name="neptune_BezierCircle_27" scale={49.927}>
                <Select
                  enabled={hovered === "Neptune"}
                  onPointerOver={over("Neptune")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="neptune_25"
                    position={[0.15, 0, 0.985]}
                    rotation={[0, 0, 0]}
                    scale={0.024}
                  >
                    <mesh
                      name="Object_28"
                      geometry={nodes.Object_28.geometry}
                      material={materials.neptune}
                    />
                  </group>
                </Select>
              </group>
              <group name="pluto_BezierCircle_30" scale={54.22}>
                <Select
                  enabled={hovered === "Pluto"}
                  onPointerOver={over("Pluto")}
                  onPointerOut={() => debouncedHover(null)}
                >
                  <group
                    name="pluto_28"
                    position={[-0.5, 0, 0.867]}
                    rotation={[0, 0, 0]}
                    scale={0.004}
                  >
                    <mesh
                      name="Object_31"
                      geometry={nodes.Object_31.geometry}
                      material={materials.pluto}
                    />
                  </group>
                </Select>
              </group>
              {/* moon ring */}
              <group
                name="moon_BezierCircle_33"
                position={[11.5, 6.426, 11.3]}
                scale={-1.879}
              >
                13.3, 6.426, 8.9
                <group
                  name="moon_31"
                  position={[0, 0, 1]}
                  rotation={[-0.023, 0, Math.PI / 2]}
                  scale={0.096}
                >
                  <mesh
                    name="Object_34"
                    geometry={nodes.Object_34.geometry}
                    material={materials.moon}
                  />
                </group>
              </group>
              <group name="mercury_BezierCircle001_34" scale={9.695}>
                <mesh
                  name="Object_36"
                  geometry={nodes.Object_36.geometry}
                  material={materials.Material}
                />
              </group>
              <group
                name="venus_BezierCircle001_35"
                rotation={[0, 0.028, 0]}
                scale={12.235}
              >
                <mesh
                  name="Object_38"
                  geometry={nodes.Object_38.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="erath_BezierCircle001_36" scale={16.115}>
                <mesh
                  name="Object_40"
                  geometry={nodes.Object_40.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="mars_BezierCircle001_37" scale={20.451}>
                <mesh
                  name="Object_42"
                  geometry={nodes.Object_42.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="jupiter_BezierCircle001_38" scale={28.775}>
                <mesh
                  name="Object_44"
                  geometry={nodes.Object_44.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="saturn_BezierCircle001_39" scale={36.61}>
                <mesh
                  name="Object_46"
                  geometry={nodes.Object_46.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="uranus_BezierCircle001_40" scale={44.26}>
                <mesh
                  name="Object_48"
                  geometry={nodes.Object_48.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="neptune_BezierCircle001_41" scale={49.927}>
                <mesh
                  name="Object_50"
                  geometry={nodes.Object_50.geometry}
                  material={materials.Material}
                />
              </group>
              <group name="pluto_BezierCircle001_42" scale={54.22}>
                <mesh
                  name="Object_52"
                  geometry={nodes.Object_52.geometry}
                  material={materials.Material}
                />
              </group>
              {/* moon ring object */}
              <group
                name="moon_BezierCircle001_43"
                position={[13.3, 6.426, 8.9]}
                scale={-1.879}
              >
                <mesh
                  name="Object_54"
                  geometry={nodes.Object_54.geometry}
                  material={materials.Material}
                />
              </group>
              <Select
                enabled={hovered === "Sol"}
                onPointerOver={over("Sol")}
                onPointerOut={() => debouncedHover(null)}
              >
                <group name="sun_53" scale={5.997}>
                  <mesh
                    name="Object_56"
                    geometry={nodes.Object_56.geometry}
                    material={materials.material}
                  />
                </group>
              </Select>
            </group>
          </group>
        </group>
      </group>
      {hovered && <Names name={hovered} position={[0, 10, 0]} />}
    </group>
  );
}

useGLTF.preload("./models/solar_system_animation.glb");
